...
The server-side part of a plugin runs in an isolated environment (a WASM runtime): it has no direct access to the file system or the network, and every call goes through a controlled panel interface. Privileged calls — managing servers, running commands and working with files on nodes, secrets, receiving events — are additionally gated on the permissions the administrator has granted the plugin: installation grants exactly what the plugin's manifest declares, and the grants are edited in the **Permissions** dialog (see [Plugin permissions](/en/plugins/management.html#plugin-permissions)). Whether the grants are applied is decided by `PLUGINS_PERMISSIONS_ENFORCE` — in 4.5.0 it is `false` by default.

Outbound SSH connections from the panel are disabled until the operator sets `PLUGINS_SSH_ENABLED=true`; plugin secrets are encrypted with the panel's `ENCRYPTION_KEY`; expensive host functions are rate limited per plugin; the node paths a plugin may access can be confined with `PLUGINS_NODEFS_PATH_POLICY`. Only a panel administrator can install and remove plugins. When installing from the catalog, the panel verifies the SHA-256 hash of the downloaded file.
