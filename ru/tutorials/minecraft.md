...
* В поле "Модификация" рекомендуем выбрать "Multicore" — она позволяет легко управлять версиями серверов Minecraft.
...
Модификация "Multicore" предоставляет следующие настройки:

| Настройка                           | Тип                                        | По умолчанию | Описание                                                                                                       |
|-------------------------------------|--------------------------------------------|--------------|----------------------------------------------------------------------------------------------------------------|
| Minecraft version                   | Список (допускается произвольное значение) | 1.20.4       | Версия сервера. Выберите значение из списка или введите любую другую версию                                    |
| Mod                                 | Список                                     | Vanilla      | Ядро сервера: Vanilla, Paper, Forge, Fabric, Spigot, CraftBukkit, Cauldron, Waterfall, Velocity или BungeeCord |
| Mod version                         | Строка                                     | —            | Версия выбранного ядра, например сборка Forge                                                                  |
| Memory. Max heap size (Xmx)         | Строка                                     | 1G           | Максимальный размер кучи Java                                                                                  |
| Min memory. Initial heap size (Xms) | Строка                                     | 512M         | Начальный размер кучи Java                                                                                     |

Все эти настройки отмечены как **Только для администраторов** и отображаются только администраторам.
...
В этом примере будет использоваться Minecraft сервер версии 1.19.4 со Spigot API для работы с плагинами.
