...
> В `--host` указывайте адрес, по которому панель будет доступна, — доменное имя или IP, назначенный
> сетевому интерфейсу самого сервера. Значение попадает в `HTTP_HOST`, из него панель выводит адрес
> прослушивания: при `--host=127.0.0.1` слушатели HTTP и gRPC поднимутся только на локальной петле,
> и подключиться не смогут ни администраторы извне, ни демоны с других машин. За NAT внешний IP не
> назначен ни одному локальному интерфейсу: указывайте здесь внутренний адрес, а внешний оставьте
> для DNS и перенаправления портов.
