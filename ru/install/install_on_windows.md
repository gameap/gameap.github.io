---
title: Установка на Windows
description: "Установка панели GameAP на Windows через gameapctl: поддерживаемые версии системы, загрузка, запуск установщика и завершение настройки."
layout: default
lang: ru
category: Установка GameAP
order: 101
---

## Поддерживаемые версии

| Версия      | Поддерживается | 
|-------------|----------------|
| Server 2025 | ✔              |
| Server 2022 | ✔              |
| Server 2019 | ✔              |
| 11          | ✔              |
| 10          | ✔              |


## Загрузка GameAP Control

Необходимо скачать утилиту GameAP Control (gameapctl) для управления окружением GameAP.

Для этого необходимо перейти на страницу релизов gameapctl в Github: 
[https://github.com/gameap/gameapctl/releases](https://github.com/gameap/gameapctl/releases)

Выберите там последний релиз и кликните по нему

<video width="1280" height="720" controls>
  <source src="/images/en/gameapctl/download.webm" type="video/webm">
  Your browser does not support the video tag.
</video>

После этого найдите версию подходящую вам. Наиболее популярной является архитектура
Windows AMD64, поэтому скорее всего вам необходимо скачать именно этот архив:

![Выбор архива gameapctl для Windows AMD64 на странице релизов](/images/en/gameapctl/download_release_windows_amd64.png)

## Установка панели с использованием GameAP Control UI

После того, как вы скачали архив с утилитой gameapctl, запустите его.

Откроется окно браузера, в котором в разделе Web/API необходимо нажать "Install".

<video width="1280" height="720" controls>
  <source src="/images/en/gameapctl/windows-install.webm" type="video/webm">
  Your browser does not support the video tag.
</video>

> Интерфейс gameapctl устанавливает и обновляет готовую релизную сборку; возможности собрать
> панель из исходников в нём нет. Для сборки из исходников на Windows используйте командную
> строку: `gameapctl.exe panel install --github`, при необходимости с `--branch=<ветка>`. Go и
> Node.js gameapctl устанавливает сам; замечания об инструментарии Go и о последующих обновлениях
> из раздела [Develop версия](/ru/install/install_on_linux.html#develop-версия) относятся и сюда.

### Параметры установки 

Укажите необходимые данные для установки

![Форма параметров установки панели в интерфейсе gameapctl](/images/en/gameapctl/ui_gameap_installation.png)

#### Хост

Укажите домен или IP адрес, по которому будет доступна панель.

В случае IP адреса, должен быть указан адрес, который назначен сетевому интерфейсу на VDS.
Если ваша сеть использует NAT, то не указывайте внешний IP,
а указывайте внутренний, затем сконфигурируйте перенаправление портов.

Домен может быть указан любой, но не забудьте настроить DNS.

Пример корректных значений:
* 10.182.104.8
* 10.182.104.8:2080
* example.com
* http://example.com

#### База данных

База данных, где будут храниться данные: пользователи, сведения о серверах и тд.
На Windows установщик заранее выбирает **SQLite**; остальные варианты по-прежнему доступны:
* [SQLite](https://www.sqlite.org/). Выбрана по умолчанию. Достаточно, если нагрузка на ваш сервер планируется небольшой и вы не планируете использовать более 10 игровых серверов.
* [PostgreSQL](https://www.postgresql.org/). Рекомендуется для больших проектов, с большим количеством игровых серверов 
  и пользователей.
* [MySQL](https://www.mysql.com/)/[MariaDB](https://mariadb.org/)

#### Установка GameAP Daemon

Помимо самой Web панели вы можете ещё установить серверную часть GameAP Daemon, которая
отвечает за работу с игровыми серверами.

### Окончание установки

Дождитесь окончания установки. Некоторые этапы 
могут занимать продолжительное время.

После окончания не забудьте сохранить данные для входа 
и данные от базы, которые будут указаны.

![Данные для входа в панель, показанные по завершении установки](/images/en/gameapctl/gameap_finished_installation.png)

## HTTPS

Одна команда переводит панель на HTTPS с самоподписанным сертификатом:

```powershell
C:\path\to\gameapctl.exe panel https enable
```

Сертификат и ключ хранятся в `C:\gameap\web\certs`. Поскольку сертификат самоподписанный,
браузеры не доверяют ему, пока он не добавлен в хранилище доверенных сертификатов на каждой
машине, с которой открывают панель. Флаги, собственный сертификат и Let's Encrypt описаны на
странице [HTTPS и сертификаты](/ru/https.html).
