...
При включённом ACME запрос, чей заголовок `Host` не совпадает ни с одной записью `ACME_DOMAINS`,
перенаправляется на первый домен списка без подстановки — поэтому основной домен панели стоит
указывать первым. Записи с подстановкой (`*.example.com`) никогда не используются как цель
перенаправления; если в списке только они, хост берётся из запроса.
