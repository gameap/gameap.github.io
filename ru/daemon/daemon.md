...
Для `docker` и `podman` в этом же блоке можно задать параметры контейнера (`image`,
...
модификации и метаданных игры. Не задавайте здесь `container_name`: он берётся по той же цепочке,
и все серверы на узле получат одно и то же имя контейнера. Список ключей контейнера и примеры — на
странице [Менеджеры процессов](/ru/daemon/process_managers.html).
