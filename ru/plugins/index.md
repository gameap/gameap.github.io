...
Серверная часть плагина выполняется в изолированном окружении (WASM-рантайм): у неё нет прямого доступа к файловой системе и сети, все обращения идут через контролируемый интерфейс панели. Привилегированные вызовы — управление серверами, выполнение команд и работа с файлами на выделенных серверах, секреты, получение событий — дополнительно проверяются по правам, которые администратор выдал плагину: при установке выдаются объявленные в манифесте плагина права, которые известны панели, а редактируются права в окне **«Права»** (см. [Права плагина](/ru/plugins/management.html#права-плагина)). Применяются ли права, определяет `PLUGINS_PERMISSIONS_ENFORCE` — в 4.5.0 по умолчанию `false`.

Исходящие SSH-подключения с панели отключены, пока оператор не установит `PLUGINS_SSH_ENABLED=true`; секреты плагинов шифруются ключом панели `ENCRYPTION_KEY`, а без ключа запись секрета отклоняется, пока оператор не отключит это требование переменной `PLUGINS_SECRETS_REQUIRE_ENCRYPTION=false` (тогда секреты сохраняются в открытом виде); ресурсоёмкие host-функции ограничены по частоте вызовов для каждого плагина; пути на выделенных серверах, доступные плагину, можно ограничить политикой `PLUGINS_NODEFS_PATH_POLICY`. Устанавливать и удалять плагины может только администратор панели. При установке из каталога панель проверяет SHA-256-хеш скачанного файла.

> Изоляция распространяется только на WASM-часть. **Фронтенд плагина песочницей не ограничен** — это обычный JavaScript в том же источнике, что и панель, с доступом к странице и к API от имени текущего пользователя. Подробнее — [Фронтенд плагина](/ru/plugins/frontend.html#как-встраивается-фронтенд).

Отсюда главное правило: плагин получает столько же прав, сколько имеет установивший его администратор. Ставьте плагины только из источников, которым доверяете.
...
* [Установка и управление](/ru/plugins/management.html) — установка плагинов из каталога и из локального файла (с предпросмотром прав), обновление, удаление, права плагинов, автоматическое восстановление, работа на нескольких экземплярах, переменные окружения.
...
* [Фронтенд плагина](/ru/plugins/frontend.html) — встраивание интерфейса плагина в панель, манифест `PluginDefinition`, 20 UI-слотов, редакторы файлов, SDK, локальная отладка.
